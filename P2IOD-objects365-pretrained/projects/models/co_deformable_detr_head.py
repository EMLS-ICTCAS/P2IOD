...
class CoDeformDETRHead(DETRHead):
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            # 这一步把cls_score重新resize成为一个一维的，为了在所有得分里计算前max_per_img
            scores, indexes = cls_score.view(-1).topk(max_per_img)
            # 除num_classes取余，得到前max_per_img序号对应的类别。
            det_labels = indexes % self.num_classes
            # 除num_classes取整，得到前max_per_img序号对应的实际框编号。
            bbox_index = indexes // self.num_classes
            # 取出前max_per_img序号对应框的回归结果
        ...
        # 对于前max_per_img的框，筛选出大于score_thr的框
        ...
        # det_bboxes中有5个维度，前4个是bbox的x1y1x2y2像素坐标，后一个是score, det_labels有一个维度，是类别
        return det_bboxes, det_labels
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False, return_encoder_output=False):
        ...
        outputs_classes, outputs_coords, topk_score, topk_anchor, enc_outs, _, _ = self.forward(feats, img_metas)
