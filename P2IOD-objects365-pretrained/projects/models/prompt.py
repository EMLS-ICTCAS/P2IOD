...
class PromptMLP(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            non_linearity,
            nn.Linear(self.hidden_features, self.out_features * self.num, bias=bias),
        )
        if dropout > 0.0:
    ...
    def forward(self, x: torch.Tensor):
        bsz = x.size(0)
        out = self.block(x)
        out = out.reshape(bsz, self.num, self.out_features)
...
class ContinuousPrompt(nn.Module):
    def __init__(
        ...
        self.sparse_prompt = True
        self.lambda_l1_norm = 0.000003
    ...
    def forward(self, x_querry: torch.Tensor):
        ...
        for i, module in enumerate(self.memory_list):
            x = x_querry  # bsz, ndim
            if self.sparse_prompt:
                l1_norm_losses += module.l1_regularization(self.lambda_l1_norm)
