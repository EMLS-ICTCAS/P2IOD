...
from .prompt import ContinuousPrompt
...
class DinoTransformerDecoder(DeformableDetrTransformerDecoder):
    ...
    def forward(self,
        ...
        # 循环各个decoder layer
        for lid, layer in enumerate(self.layers):
            #没看懂valid_ratios的作用，对参考点做了个变换，将维度也增加了，但比较细节，之后再看
            if reference_points.shape[-1] == 4:
            ...
            #对于第一层，output指的是初始化的embbeding内容查询，query_pos是位置查询
            query_pos = query_pos.permute(1, 0, 2)
            if p_list is None:
                output = layer(
                    output,
...
class CoDinoTransformer(CoDeformableDetrTransformer):
    ...
    def forward(self,
        ...
        #memory 是encoder的输出，转换后为[B, seq_lenth, C]
        memory = self.encoder(
            query=feat_flatten,
            key=None,
            value=None,
        ...
        # output_memory和memory的区别是，output_memory根据掩码（padding mask）和无效区域来更新memory，置零了一些区域
        output_memory, output_proposals = self.gen_encoder_output_proposals(
            memory, mask_flatten, spatial_shapes)
        # 这里num_layers是指取最后一层的cls_branch
        enc_outputs_class = cls_branches[self.decoder.num_layers](
            output_memory)
        ...
        # 根据topk分类值，选择topk_score（最大topk对应得分）、topk_coords_unact（最大topk对应未激活坐标）、topk_anchor（最大topk对应激活坐标）
        ...
        #self.query_embed是正态初始化的embbeding，repeat将查询扩展为batchsize, transpose得到形状为(bs,num_queries,embed_dim),num_queryies=1500
        ...
        if use_prompts:
            with torch.no_grad():
                ...
                inter_states_pretrain, inter_references_pretrain = self.decoder(
                    query=query,
                    key=None,
                    value=memory,
                    key_padding_mask=mask_flatten,
                    ...
                    p_list=None,
                    **kwargs)
                final_inter_state_pretrain = inter_states_pretrain[-1].mean(dim=0)
            p_list = self.prompts.forward(final_inter_state_pretrain)
        else:
            p_list = None

        query = query.permute(1, 0, 2)
        # 拼接去噪的query和初始化的query；值得注意的是初始化的query在后面，而不是前面；reference_points是每个query对应的坐标，由gt加噪和topk_coords_unact合并得到
        ...
        #返回的inter_states是每一层的query，inter_references是每一层query对应的参考点
        inter_states, inter_references = self.decoder(
            query=query,
            key=None,
            value=memory,
        ...
        #返回的inter_states是每一层的query，inter_references是每一层query对应的参考点，topk_score最大topk对应得分,topk_anchor最大topk对应激活坐标, memory 是encoder的输出，转换后为[B, seq_lenth, C]
        return inter_states, inter_references_out, topk_score, topk_anchor, memory, inter_states_pretrain, inter_references_pretrain
