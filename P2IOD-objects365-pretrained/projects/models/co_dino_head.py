...
class CoDINOHead(CoDeformDETRHead):
    ...
    def BG_thresholding(self, gt_bboxes, gt_labels, result_list, score_thr=0.65, bg_topk=5, previously_seen_classes=0):

        bboxes_with_pesudo_label = []
        labels_with_pesudo_label = []

        for batch_id in range(len(gt_bboxes)):

            bbox_pretrain, label_pretrain = result_list[batch_id]
            gt_bbox = gt_bboxes[batch_id]
            gt_label = gt_labels[batch_id]

            bbox_coordinates_pretrain, bbox_scores_pretrain = bbox_pretrain.split([4, 1], dim=1)
            bbox_scores_pretrain = bbox_scores_pretrain.squeeze(1)

            # 选择大于阈值的
            valid_mask_pretrain = bbox_scores_pretrain > score_thr
            bbox_scores_pretrain = bbox_scores_pretrain[valid_mask_pretrain]
            bbox_coordinates_pretrain = bbox_coordinates_pretrain[valid_mask_pretrain]
            label_pretrain = label_pretrain[valid_mask_pretrain]

            # 仅保留之前任务的类别
            label_mask = label_pretrain < previously_seen_classes
            bbox_scores_pretrain = bbox_scores_pretrain[label_mask]
            bbox_coordinates_pretrain = bbox_coordinates_pretrain[label_mask]
            label_pretrain = label_pretrain[label_mask]

            # 选前topk个
            if bbox_scores_pretrain.numel() > 0:
                topk = min(bg_topk, bbox_scores_pretrain.size(0))
                topk_scores, topk_indices = torch.topk(bbox_scores_pretrain, topk)
                bbox_coordinates_pretrain = bbox_coordinates_pretrain[topk_indices]
                label_pretrain = label_pretrain[topk_indices]

            gt_bbox = torch.cat((gt_bbox, bbox_coordinates_pretrain), dim=0)
            gt_label = torch.cat((gt_label, label_pretrain), dim=0)

            bboxes_with_pesudo_label.append(gt_bbox)
            labels_with_pesudo_label.append(gt_label)

        return bboxes_with_pesudo_label, labels_with_pesudo_label
    ...
    def forward_train(self,
        ...
        if pesudo_label:
            ...
            with torch.no_grad():
                ...
                result_list = self.get_bboxes(*outs_pretrain, img_metas=img_metas, rescale=False, with_nms=False)
                gt_bboxes_with_pesudo_label, gt_labels_with_pesudo_label = self.BG_thresholding(gt_bboxes, gt_labels, result_list, previously_seen_classes=self.previously_seen_classes)
    ...
    def forward(self,
        ...
        query_embeds = None
        #这里过encoder和decoder, 返回的hs是每一层的query，inter_references是每一层query对应的参考点，topk_score最大topk对应得分,topk_anchor最大topk对应激活坐标, enc_outputs 是encoder的输出，转换后为[B, seq_lenth, C]
        hs, inter_references, topk_score, topk_anchor, enc_outputs, hs_pretrain, inter_references_pretrain = \
            self.transformer(
                mlvl_feats,
                mlvl_masks,
                query_embeds,
        ...
        # outs保存了encoder的输出，并从序列还原为了特征层的形式
        start = 0
        ...
        hs_pretrain = hs_pretrain.permute(0, 2, 1, 3)
        ...
        # 这里对6个decoder的query分别过cls_branch和reg_branch
        ...
        with torch.no_grad():
            outputs_classes_pretrain = []
            outputs_coords_pretrain = []

            # 这里对6个decoder的query分别过cls_branch和reg_branch
            for lvl in range(hs_pretrain.shape[0]):
                reference_pretrain = inter_references_pretrain[lvl]
                reference_pretrain = inverse_sigmoid(reference_pretrain, eps=1e-3)
                outputs_class_pretrain = self.cls_branches[lvl](hs_pretrain[lvl])
                tmp_pretrain = self.reg_branches[lvl](hs_pretrain[lvl])
                if reference_pretrain.shape[-1] == 4:
                    tmp_pretrain += reference_pretrain
                else:
                    assert reference_pretrain.shape[-1] == 2
                    tmp_pretrain[..., :2] += reference_pretrain
                outputs_coord_pretrain = tmp_pretrain.sigmoid()
                outputs_classes_pretrain.append(outputs_class_pretrain)
                outputs_coords_pretrain.append(outputs_coord_pretrain)

            outputs_classes_pretrain = torch.stack(outputs_classes_pretrain)
            outputs_coords_pretrain = torch.stack(outputs_coords_pretrain)


        # 返回的内容：1、6个decoder query的分类输出。2、6个decoder query的回归输出。3、topk_score（encoder最大topk对应得分）4、topk_anchor（encoder最大topk对应激活坐标）5、outs保存了encoder的输出，并从序列还原为了特征层的形式
        return outputs_classes, outputs_coords, topk_score, topk_anchor, outs, outputs_classes_pretrain, outputs_coords_pretrain
    ...
    def loss(self,
             ...
             img_metas,
             dn_meta=None,
             gt_bboxes_list_with_pesudo_label=None,
             gt_labels_list_with_pesudo_label=None,
             gt_bboxes_ignore=None):
        ...
        if enc_topk_scores is not None:
            if gt_bboxes_list_with_pesudo_label is None:
                ...
                enc_loss_cls, enc_losses_bbox, enc_losses_iou = \
                    self.loss_single(enc_topk_scores, enc_topk_anchors,
                                    gt_bboxes_list_with_pesudo_label, gt_labels_list_with_pesudo_label,
                                    img_metas, gt_bboxes_ignore)
        ...
        if gt_bboxes_list_with_pesudo_label is None:
            ...
            all_gt_bboxes_list_with_pesudo_label = [gt_bboxes_list_with_pesudo_label for _ in range(num_dec_layers)]
            all_gt_labels_list_with_pesudo_label = [gt_labels_list_with_pesudo_label for _ in range(num_dec_layers)]
            all_gt_bboxes_ignore_list = [
                gt_bboxes_ignore for _ in range(num_dec_layers)
            ]
            img_metas_list = [img_metas for _ in range(num_dec_layers)]
            losses_cls, losses_bbox, losses_iou = multi_apply(
                self.loss_single, all_cls_scores, all_bbox_preds,
                all_gt_bboxes_list_with_pesudo_label, all_gt_labels_list_with_pesudo_label, img_metas_list,
                all_gt_bboxes_ignore_list)
