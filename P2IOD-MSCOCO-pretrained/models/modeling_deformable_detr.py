...
from .prompt import ParameterizedPrompt
    ...
    def __init__(self, config):
        ...
        if config.use_timm_backbone:
            ...
            backbone = create_model(
                ...
                pretrained_cfg_overlay=dict(file='resnet50.a1_in1k/pytorch_model.bin'),
                features_only=True,
...
class DeformableDetrModel(DeformableDetrPreTrainedModel):
    def __init__(self, config: DeformableDetrConfig):
        ...
        if config.use_prompts:
            self.prompts = ParameterizedPrompt(args=config)
...
class DeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig, default=True, log_file=None):
        ...
        self.matcher = DeformableDetrHungarianMatcher(
                class_cost=config.class_cost, bbox_cost=config.bbox_cost, giou_cost=config.giou_cost,focal_alpha=config.focal_alpha , focal_gamma=config.focal_gamma
            )
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.two_stage:
                weight_dict.update({k + f'_enc': v for k, v in weight_dict.items()})
...
class DeformableDetrLoss(nn.Module):
    ...
    def __init__(self, matcher, num_classes, focal_alpha, focal_gamma, losses, invalid_cls_logits):
        super().__init__()
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        loss_ce = (
            sigmoid_focal_loss(source_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=self.focal_gamma)
...
class DeformableDetrHungarianMatcher(nn.Module):
    ...
    def __init__(self, class_cost: float = 1, bbox_cost: float = 1, giou_cost: float = 1, focal_alpha: float = 0.25, focal_gamma: float = 2.0):
        super().__init__()
