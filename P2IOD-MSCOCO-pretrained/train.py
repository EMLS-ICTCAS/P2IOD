...
from fuse_method.value_based_fuse_consist_average import ValueBasedFuse
...
def get_args_parser():
    ...
    parser.add_argument('--repo_name', default="coco_pretrained_Deformable_DETR", type=str, 
    ...
    parser.add_argument('--checkpoint_dir', default='', 
                        help='Directory for test checkpoints and resume checkpoints')
    parser.add_argument('--checkpoint_task1_name', default='checkpoint05.pth', 
                        help='Base checkpoint directory')
    parser.add_argument('--checkpoint_other_task_name', default='fused_model.pth', 
                        help='Next checkpoint to load')

    # Dataset paths
    parser.add_argument('--train_img_dir', default='datasets/voc07/train', type=str, 
                        help='Training images directory')
    parser.add_argument('--test_img_dir', default='datasets/voc07/val', type=str, 
                        help='Validation images directory')
    parser.add_argument('--task_ann_dir', default='split_dataset/voc_5_5_5_5', type=str, 
                        help='Directory for task annotations')
    parser.add_argument('--split_task', default='voc_5_5_5_5', type=str, 
    ...
    parser.add_argument('--topk_percent_new', default=0.5, type=float, 
                        help='Fuse top-l percent parameter')
    
    # Sparse prompt
    parser.add_argument('--sparse_prompt', default=True, action='store_true', 
                        help='Add sparse loss to regulate the sparse prompt')
    parser.add_argument('--lambda_l1_norm', default=0.00001, type=float, 
...
def main(args):
    ...
    logger = CSVLogger(save_dir=args.output_dir, name="lightning_logs", flush_logs_every_n_steps=1)

    fuse_method = ValueBasedFuse(args.topk_percent_ori, args.topk_percent_new)
    ...
    for task_id in range(args.start_task, args.n_tasks+1):
        ...
        if task_id>1:
            ...
            if task_id == 2:
                trainer.resume(os.path.join(prev_task,args.checkpoint_task1_name))
            else:
                trainer.resume(os.path.join(prev_task,args.checkpoint_other_task_name))
        else:
            if args.repo_name:
                trainer.resume()
            args.resume=0
        ...
        # Save the parameters of parametrized prompted before training each task.
        if args.parameterized_prompt_fusion:
            if task_id==1:
                prompt_mlp_init = trainer.model.model.prompts.get_prompt_memory_params()
                save_prompt_mlp_init_pth = out_dir_root +'/init_prompt_mlp.pth'
                torch.save(prompt_mlp_init, save_prompt_mlp_init_pth)
            else:
                prompt_mlp_init = torch.load(os.path.join(out_dir_root+'/init_prompt_mlp.pth'))
                prompt_mlp_ori = trainer.model.model.prompts.get_prompt_memory_params()
                save_prompt_mlp_pth = out_dir_root+'/Task_'+str(task_id)+'/ori_prompt_mlp.pth'
                torch.save(prompt_mlp_ori, save_prompt_mlp_pth)
        ...
        if task_id>1:
            ...
            # Fuse the parameterized prompts and additionally validate the performance of the fused parameterized prompts on both the current and previous tasks
            if args.parameterized_prompt_fusion:
                prompt_mlp_new = trainer.model.model.prompts.get_prompt_memory_params()
                merged_prompt_mlp = [
                    fuse_method.merge(ori, new, init)
                    for ori, new, init in zip(prompt_mlp_ori, prompt_mlp_new, prompt_mlp_init)
                ]
                trainer.model.model.prompts.set_memory_params(merged_prompt_mlp)
                ...
                torch.save({
                        ...
                        #'args': self.args,
                    }, os.path.join(args.output_dir, args.checkpoint_other_task_name))
                with open(args.output_dir+'/stats.txt', 'a') as f:
                    f.writelines('Having fusion prompt mlp\n')

                ####################### Evaluating on current classes ###################################################
                tst_ann = os.path.join(args.task_ann_dir,'test_task_'+str(task_id)+'.json')
                test_dataset = CocoDetection(img_folder=args.test_img_dir, 
                                            ann_file=tst_ann, processor=processor)            
                test_dataloader_cur = DataLoader(test_dataset, collate_fn=test_dataset.collate_fn, batch_size=args.batch_size,
                                            num_workers=args.num_workers)
                args.task = str(task_id)
                coco_evaluator = CocoEvaluator(test_dataset.coco, args.iou_types)
                local_evaluator = Evaluator(processor=processor, test_dataset=test_dataset,test_dataloader=test_dataloader_cur,
                                coco_evaluator=coco_evaluator,args=args,task_label2name=args.task_label2name, task_name='cur')
                ...
                trainer.evaluator.model = trainer.model
                trainer.eval_mode = True
                pyl_trainer.validate(trainer,test_dataloader_cur)
