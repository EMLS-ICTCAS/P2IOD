...
def select_image(Imagesets_path, image_path, out_dir, train_or_val):
    with open(Imagesets_path, 'r') as f: 
        lines = f.readlines()
        splitlines = [x.strip('\n') for x in lines]
        out_image_dir = os.path.join(out_dir,train_or_val)
        mkdir_or_exist(out_image_dir)
        for i, line  in enumerate(splitlines):
            img_name = line + '.jpg'
            img_path = os.path.join(image_path,img_name)
            shutil.copy(img_path, out_image_dir)
...
def main():
    ...
    for year in years:
        if year == '2007':
            prefix = 'voc07/annotations/'
        elif year == '2012':
            prefix = 'voc12/annotations/'
        for split in ['trainval']:
            dataset_name = prefix + 'instances_train'
            Imagesets_path = osp.join(devkit_path, 'VOC'+year+'/ImageSets/Main/trainval.txt')
            image_path = osp.join(devkit_path, 'VOC'+year+'/JPEGImages')
            print(f'processing {dataset_name} ...')
            cvt_annotations(devkit_path, year, split,
                            osp.join(out_dir, dataset_name + out_fmt))
            print(Imagesets_path)
            print(image_path)
            print(osp.join(out_dir,'voc07'))
            select_image(Imagesets_path, image_path, osp.join(out_dir,'voc07'), 'train')
        if not isinstance(year, list):
            dataset_name = prefix + 'instances_val'
            Imagesets_path = osp.join(devkit_path, 'VOC'+year+'/ImageSets/Main/test.txt')
            image_path = osp.join(devkit_path, 'VOC'+year+'/JPEGImages')
            print(f'processing {dataset_name} ...')
            cvt_annotations(devkit_path, year, 'test',
                            osp.join(out_dir, dataset_name + out_fmt))
            select_image(Imagesets_path, image_path, osp.join(out_dir,'voc07'), 'val')
